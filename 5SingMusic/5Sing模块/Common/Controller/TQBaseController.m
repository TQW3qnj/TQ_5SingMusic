...
@interface TQBaseController ()

@end

@implementation TQBaseController
...
-(void)creatMyNavUI{
    ...
    //  右侧按钮
    UIButton *rightButton =[self creatButtonWithFrame:CGRectMake(SCREENW-35, 25, 30, 30) Image:@"music6" SelectImage:@"music6" SuperView:_navView];
    [rightButton addTarget:self action:@selector(listenMusic:) forControlEvents:UIControlEventTouchUpInside];
    
    //中间标签按钮
    _btnArray =[[NSMutableArray alloc]init];
    CGFloat x =(SCREENW-80-self.btnTitleArray.count*60)/(self.btnTitleArray.count+1);
...
-(void)creatMyScrollUI{
    ...
    _myScrollView =[[UIScrollView alloc]initWithFrame:CGRectMake(0, 64, SCREENW, SCREENH)];
    ...
    _myScrollView.contentSize =CGSizeMake(_btnTitleArray.count*SCREENW, SCREENH);
    ...
    for (int i =0; i<_btnTitleArray.count; i++) {
        if ([sender.titleLabel.text isEqualToString:_btnTitleArray[i]]) {
            [UIView animateWithDuration:0.2 animations:^{
                 _myScrollView.contentOffset=CGPointMake(i*SCREENW, 0);
            }];
            sender.selected =YES;
